...
export async function GET() {
  try {
    ...
    const url = `${config.url}/get/editor_data`;
    ...
    if (data.result) {
      ...
      console.log('Editor data fetched from Redis successfully');
      return NextResponse.json(parsed);
    }
    console.log('No editor data found in Redis');
    return NextResponse.json({});
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const url = `${config.url}/set/editor_data`;
    
    console.log('Saving editor data to Redis...');
    ...
    console.log('Editor data saved to Redis successfully');
