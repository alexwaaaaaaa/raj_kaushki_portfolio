...
export function EducationTab(): React.ReactElement {
  const experiences = usePortfolioStore((s) => s.data.education || []);
  const setExperience = usePortfolioStore((s) => s.updateEducation);
  ...
  const addNew = () => {
    ...
    setEditForm({
      id: `edu_${Date.now()}`,
      role: '', company: '', location: '', duration: '', type: 'Education', bullets: ['']
    });
  };
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto h-full overflow-auto custom-scrollbar">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-2xl font-display font-bold text-[var(--text-primary)]">Education</h2>
          <p className="text-[var(--text-muted)] text-sm">Manage your education history.</p>
        </div>
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div className="col-span-2">
        <label className="text-xs text-[var(--text-secondary)] mb-1 block">Degree / Certificate</label>
        <input name="role" value={state.role} onChange={handleChange} className="input w-full" placeholder="e.g. Bachelor of Technology" />
      </div>
      <div>
        <label className="text-xs text-[var(--text-secondary)] mb-1 block">Institution</label>
        <input name="company" value={state.company} onChange={handleChange} className="input w-full" placeholder="e.g. XYZ University" />
      </div>
      <div>
      ...
      <div>
        ...
        <input name="type" value={state.type} onChange={handleChange} className="input w-full" placeholder="e.g. Education" />
      </div>
